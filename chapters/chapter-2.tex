...
\begin{longtable}{|>{\raggedleft\arraybackslash}p{0.03\literatureTableWidth}|>{\raggedright\arraybackslash}p{0.10\literatureTableWidth}|>{\raggedright\arraybackslash}p{0.14\literatureTableWidth}|>{\raggedright\arraybackslash}p{0.27\literatureTableWidth}|>{\raggedright\arraybackslash}p{0.21\literatureTableWidth}|>{\raggedright\arraybackslash}p{0.25\literatureTableWidth}|}
...
\begin{figure}[H]
	...
	\begin{subfigure}{0.48\textwidth}
		\centering
		\fbox{\includegraphics[width=0.75\textwidth]{figure/aku.png}}
		\caption{Isyarat SIBI kata aku}
	\end{subfigure}
	\hfill
	\begin{subfigure}{0.48\textwidth}
		\centering
		\fbox{\includegraphics[width=0.75\textwidth]{figure/sekolah.png}}
...
Gambar \ref{fig:transformer-architecture} memperlihatkan alur lengkap Transformer yang terdiri dari lima bagian utama. Bagian (1) menunjukkan urutan masukan yang terlebih dahulu dipetakan ke \textit{embedding} dan digabungkan dengan \textit{positional encoding}. Bagian (2) adalah encoder yang menyusun representasi kontekstual melalui \textit{multi-head attention}, \textit{feed-forward network}, \textit{residual connection}, dan \textit{layer normalization}. Bagian (3) menunjukkan masukan decoder yang telah digeser (\textit{shifted right}) lalu diberi \textit{embedding} dan \textit{positional encoding}. Bagian (4) adalah decoder yang memadukan \textit{masked self-attention} dengan perhatian silang terhadap keluaran encoder. Bagian (5) merupakan lapisan linier dan \textit{softmax} yang menghasilkan probabilitas keluaran. Penomoran ini digunakan untuk menegaskan urutan aliran data dari masukan hingga prediksi akhir.\par
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[x=1cm,y=1cm,
      ...
      num/.style = {circle, draw=black, fill=yellow!35, minimum size=0.52cm, inner sep=0pt, font=\bfseries\scriptsize},
      arr/.style = {-{Stealth[length=4pt]}, line width=0.6pt, black}
    ]
      % Encoder side
      ...
      % Number markers
      \node[num] at (0.2,10.8) {1};
      \node[num] at (4.0,8.2) {2};
      \node[num] at (6.3,9.8) {3};
      \node[num] at (11.9,6.8) {4};
      \node[num] at (11.7,-1.4) {5};
    \end{tikzpicture}%
    }}
...
Karena jumlah token video dapat sangat besar, Arnab dkk.\ tidak hanya mengusulkan satu rancangan ViViT, tetapi beberapa varian untuk menyeimbangkan kemampuan representasi dan efisiensi komputasi \cite{Arnab2021}. Oleh sebab itu, Gambar~\ref{fig:vivit-architecture} lebih tepat dibaca sebagai perbandingan empat versi encoder ViViT yang sama-sama berangkat dari token spasio-temporal hasil \textit{tubelet embedding}, tetapi berbeda dalam cara memodelkan hubungan ruang dan waktu. Penomoran pada gambar digunakan untuk membedakan keempat varian tersebut secara jelas.\par

\subsubsection{Arsitektur ViViT} \label{II.ViViT.2}
Secara umum, seluruh varian ViViT tetap mempertahankan komponen dasar Transformer, yaitu token video hasil \textit{tubelet embedding}, \textit{positional embedding}, blok \textit{self-attention}, \textit{feed-forward network}, serta kepala klasifikasi pada keluaran akhir \cite{Vaswani2023,Arnab2021}. Perbedaan utamanya terletak pada lokasi dan cara faktorisasi operasi perhatian ruang--waktu. Skema keempat varian tersebut ditunjukkan pada Gambar~\ref{fig:vivit-architecture}.\par
	...
	\fbox{%
		\begin{tikzpicture}
			...
			\begin{scope}[x={(img.south east)}, y={(img.north west)}]
				\node[circle, draw=black, fill=yellow!35, minimum size=0.48cm, inner sep=0pt, font=\bfseries\scriptsize] at (0.38,1) {1};
				\node[circle, draw=black, fill=yellow!35, minimum size=0.48cm, inner sep=0pt, font=\bfseries\scriptsize] at (0.61,1) {2};
				\node[circle, draw=black, fill=yellow!35, minimum size=0.48cm, inner sep=0pt, font=\bfseries\scriptsize] at (0.79,1) {3};
				\node[circle, draw=black, fill=yellow!35, minimum size=0.48cm, inner sep=0pt, font=\bfseries\scriptsize] at (0.95,1) {4};
			\end{scope}
		\end{tikzpicture}%
	}
...
Pada gambar tersebut, varian (1) adalah \textit{Transformer Encoder} standar yang memproses seluruh token video secara bersama dalam satu rangkaian blok perhatian ruang--waktu. Varian (2) adalah \textit{Factorised Encoder}, yaitu versi yang menyusun beberapa blok spasial terlebih dahulu, kemudian melanjutkannya dengan blok temporal, sehingga pemodelan relasi antar-patch dan antar-frame dilakukan secara bertahap. Varian (3) adalah \textit{Factorised Self-Attention}, di mana perhatian spasial dan temporal dipisahkan di dalam satu lapisan agar biaya komputasi lebih ringan. Varian (4) adalah \textit{Factorised Dot-Product Attention}, yaitu perhatian spasial dan temporal dihitung terpisah lalu digabungkan kembali melalui proses \textit{fuse}. Dari keempat susunan ini, varian (2) paling relevan untuk memperjelas ide dasar ViViT karena menunjukkan secara eksplisit bahwa hubungan spasial dimodelkan lebih dahulu, lalu hubungan temporal dipelajari sesudahnya. Dalam penelitian ini digunakan konfigurasi ViViT-B/16x2, yaitu \textit{backbone} ViT-Base dengan ukuran patch 16 piksel dan panjang \textit{tubelet} 2 frame yang telah di-\textit{pretrain} pada Kinetics-400 \cite{Arnab2021}. Bobot pra-latih tersebut diinisialisasi dari \textit{checkpoint} ViT berbasis ImageNet, kemudian dilanjutkan dengan pra-latih \textit{supervised} pada dataset video berskala besar \cite{Arnab2021}. Strategi ini berbeda dari VideoMAE yang mengandalkan pra-latih \textit{self-supervised} tanpa label, sehingga ViViT tetap merepresentasikan paradigma \textit{supervised video transformer} dalam penelitian ini \cite{Arnab2021,Tong2022VideoMAE}.\par
...
\begin{figure}[h]
	...
	\fbox{%
		\begin{tikzpicture}
			...
			\begin{scope}[x={(img.south east)}, y={(img.north west)}]
				\node[circle, draw=black, fill=yellow!35, minimum size=0.5cm, inner sep=0pt, font=\bfseries\scriptsize] at (0.08,0.82) {1};
				\node[circle, draw=black, fill=yellow!35, minimum size=0.5cm, inner sep=0pt, font=\bfseries\scriptsize] at (0.27,0.82) {2};
				\node[circle, draw=black, fill=yellow!35, minimum size=0.5cm, inner sep=0pt, font=\bfseries\scriptsize] at (0.42,0.82) {3};
				\node[circle, draw=black, fill=yellow!35, minimum size=0.5cm, inner sep=0pt, font=\bfseries\scriptsize] at (0.55,0.72) {4};
				\node[circle, draw=black, fill=yellow!35, minimum size=0.5cm, inner sep=0pt, font=\bfseries\scriptsize] at (0.72,0.86) {5};
			\end{scope}
		\end{tikzpicture}%
	}
...
Gambar~\ref{fig:videomae-architecture} memperlihatkan alur VideoMAE dalam lima tahap utama, mulai dari pembentukan token spasio-temporal, proses \textit{tube masking}, pemilihan token yang tetap terlihat, pengodean oleh encoder ViT 3D, hingga rekonstruksi kembali klip target oleh decoder. Penomoran pada gambar digunakan agar setiap tahap dapat diikuti secara berurutan dari kiri ke kanan.\par
\begin{enumerate}
\item Token spasio-temporal\\
Pada tahap (1), setiap klip dibagi menjadi kubus berukuran tetap yang kemudian dipetakan ke vektor token melalui \textit{cube embedding}. Proses ini menyatukan informasi ruang dan waktu menjadi barisan token yang dapat diproses oleh Transformer \cite{Tong2022VideoMAE}. Penambahan \textit{positional embedding} menjaga urutan temporal dan lokasi spasial selama atensi bersama \cite{Tong2022VideoMAE}. Representasi token inilah yang menjadi dasar seluruh tahap berikutnya.

\item \textit{Tube masking}\\
Pada tahap (2), sebagian besar token disembunyikan dengan pola masker yang konsisten sepanjang dimensi waktu. Strategi ini mencegah \textit{information leakage} antarkader sekaligus memaksa model belajar dari konteks parsial yang tetap informatif \cite{Tong2022VideoMAE}. Rasio masking yang sangat tinggi membuat VideoMAE efisien karena hanya sebagian kecil token yang benar-benar diteruskan ke encoder.

\item Token yang tetap terlihat\\
Pada tahap (3), token yang tidak termask menjadi masukan utama bagi encoder. Pemilihan token terlihat inilah yang membuat VideoMAE tetap mempertahankan konteks penting sambil memangkas panjang urutan yang harus diproses \cite{Tong2022VideoMAE}. Dengan demikian, informasi yang diteruskan ke encoder sudah merupakan subset token yang paling relevan untuk pembelajaran representasi.

\item Encoder ViT 3D\\
Pada tahap (4), encoder menggunakan \textit{joint space--time attention} untuk memodelkan hubungan spasial dan temporal pada token yang terlihat. Karena panjang urutan encoder hanya sekitar 10 persen dari token awal, biaya komputasi turun drastis tanpa kehilangan konteks global \cite{Tong2022VideoMAE}. Hasil encoder berupa representasi laten yang kaya dan menjadi dasar proses rekonstruksi maupun \textit{fine-tuning} untuk tugas hilir.

\item Decoder ringan dan rekonstruksi\\
Pada tahap (5), decoder menerima gabungan fitur encoder dan token \texttt{[MASK]} untuk merekonstruksi piksel pada lokasi yang hilang hingga membentuk kembali klip target. Fungsi kerugian yang digunakan adalah \textit{mean squared error} pada piksel termask yang dinormalisasi, sehingga gradien difokuskan pada area yang tidak terlihat encoder \cite{Tong2022VideoMAE}. Pada VideoMAE~V2, sebagian token decoder dapat dipilih lebih lanjut melalui \textit{running-cell masking} agar rekonstruksi tetap representatif namun lebih hemat memori \cite{Wang2023VideoMAEv2}.
\end{enumerate}
...
\begin{enumerate}
	...
	\begin{equationcaptioned}[eq:2.accuracy]{
		\mathrm{Accuracy} = \frac{\sum_{i=1}^{K} n_{ii}}{N} = \frac{\sum_{i=1}^{K} TP_i}{N}
	}{
...
\noindent\begin{figure}[H]
	\centering
	\input{figure/stratified_kfold_illustration.tikz}
	\caption{Ilustrasi Stratified Cross-Validation}
	\label{fig:II.stratified_kfold}
\end{figure}
\textit{Stratified Cross-Validation} (SCV) merupakan skema validasi yang membagi data ke dalam beberapa \textit{fold} sambil mempertahankan proporsi kelas yang relatif sama pada setiap bagian data. Pendekatan ini penting ketika distribusi kelas tidak seimbang, karena pembagian acak biasa dapat menghasilkan subset yang terlalu didominasi kelas tertentu dan membuat evaluasi menjadi bias. Kajian terbaru menunjukkan bahwa kualitas pembagian \textit{fold} berpengaruh langsung terhadap reliabilitas evaluasi, terutama ketika distribusi label bersifat timpang atau jarang \cite{Tiittanen2022}. Dengan menjaga komposisi label tetap proporsional, SCV memberikan estimasi performa yang lebih stabil dan lebih representatif terhadap kemampuan generalisasi model pada data baru.

Dalam literatur evaluasi model yang lebih baru, kestabilan estimasi error juga menjadi perhatian penting karena hasil \textit{k-fold cross-validation} dapat berubah akibat variasi pembagian data dan korelasi antar galat pada setiap \textit{fold} \cite{Bates2021}. Oleh karena itu, pada tugas klasifikasi seperti pengenalan gerakan bahasa isyarat, penggunaan SCV menjadi relevan karena tidak hanya memanfaatkan data secara efisien, tetapi juga membantu menjaga distribusi kelas tetap konsisten pada setiap putaran evaluasi sehingga penilaian performa model menjadi lebih andal \cite{Tiittanen2022,Bates2021}.
